...
const labels=[],pointers=new Map(),cityGroup=new T.Group(),waterGroup=new T.Group(),borders=new T.Group();
...
for(const [name,lon,lat] of cities){const point=project([lon,lat],.32),marker=new T.Mesh(new T.SphereGeometry(.06,10,8),new T.MeshBasicMaterial({color:0xffd27d}));marker.position.copy(point);cityGroup.add(marker);const el=document.createElement('span');el.className='city';el.textContent=name;$('labels').append(el);labels.push({el,point});}
...
addEventListener('keydown',e=>{if(e.key==='Escape')$('back').click();});addEventListener('resize',resize);init();
