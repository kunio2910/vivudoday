(() => {
  ...
  regionLegend.innerHTML=`<button class="region-chip all" data-region-view="Tất cả" aria-pressed="true">Tất cả</button>${Object.values(regionViews).map(r=>`<button class="region-chip ${r.slug}" data-region-view="${r.label}" aria-pressed="false">${r.label}</button>`).join('')}`;
