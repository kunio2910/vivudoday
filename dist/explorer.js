(() => {
  ...
  const state={query:'',category:'all',region:'Tất cả',regionView:null,provinceView:null,provinceMarker:null,hcmView:false,hcmMarker:null,saved:read('vne-favorites'),trip:read('vivu-trip'),savedOnly:false,view:'map',zoom:1,selected:places.some(p=>p.id===location.hash.slice(1))?location.hash.slice(1):null};
  ...
  let hcmQuery='',expandedDistrict=null;
  ...
  function renderRegionAreas(){
    regionAreas.innerHTML=state.regionView?'':Object.values(regionViews).map(r=>{const names=provinceGroups[r.label]||[];return `<button class="region-hotspot ${r.slug}" data-region-view="${r.label}" aria-label="Mở bản đồ ${r.label}"><span class="region-hover-card"><strong>${esc(r.label)}</strong><small>${esc(names.join(' · '))}</small></span></button>`;}).join('');
  ...
  function applyRegionView(){
    ...
    $('layout').classList.toggle('hcm-layout',hcm);
  ...
  let resizeTimer;
  addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{if(state.hcmView)renderHcmMap();else if(state.provinceView)renderProvinceMap();},120);});
