(() => {
  ...
  document.addEventListener('click',e=>{const thumbnail=e.target.closest('[data-province-open]');if(thumbnail){e.preventDefault();openProvince(thumbnail.dataset.provinceOpen);return;}const b=e.target.closest('button');if(!b)return;
    ...
    if(b.hasAttribute('data-province-choice')){state.provinceMarker=b.dataset.provinceChoice;state.countryChoices=null;render();return;}
     if(b.classList.contains('province-label')){state.provinceMarker=b.dataset.province;state.hcmMarker=null;state.hanoiMarker=null;state.daNangMarker=null;render();return;}if(b.hasAttribute('data-hcm-place')){state.hcmMarker=b.dataset.hcmPlace;state.hanoiMarker=null;state.daNangMarker=null;state.provinceMarker=null;state.selected=null;render();return;}if(b.hasAttribute('data-hanoi-place')){state.hanoiMarker=b.dataset.hanoiPlace;state.hcmMarker=null;state.daNangMarker=null;state.provinceMarker=null;state.selected=null;render();return;}if(b.hasAttribute('data-danang-place')){state.daNangMarker=b.dataset.danangPlace;state.hcmMarker=null;state.hanoiMarker=null;state.provinceMarker=null;state.selected=null;render();return;}if(b.hasAttribute('data-hcm-clear')){state.hcmMarker=null;render();return;}if(b.hasAttribute('data-hanoi-clear')){state.hanoiMarker=null;render();return;}if(b.hasAttribute('data-danang-clear')){state.daNangMarker=null;render();return;}if(b.dataset.province){openProvince(b.dataset.province);return;}if(b.hasAttribute('data-back-region')){openRegion(state.regionView||'Trung Bộ');return;}if(b.dataset.select)select(b.dataset.select,b.classList.contains('marker'));if(b.dataset.save){const id=b.dataset.save;state.saved=state.saved.includes(id)?state.saved.filter(x=>x!==id):[...state.saved,id];persist('vne-favorites',state.saved);render();}if(b.dataset.add){if(!state.trip.includes(b.dataset.add))state.trip.push(b.dataset.add);persist('vivu-trip',state.trip);render();notify('Đã thêm vào hành trình');}if(b.dataset.category){state.category=b.dataset.category;document.querySelectorAll('[data-category]').forEach(x=>x.setAttribute('aria-pressed',x===b));render();}});
  ...
  const regionViews={
    'Bắc Bộ':{slug:'bac-bo',label:'Bắc Bộ',description:'Núi cao, đồng bằng và biển đảo phía Bắc',scale:1.28,x:'30%',y:'18%'},
    'Trung Bộ':{slug:'trung-bo',label:'Trung Bộ',description:'Dải di sản ven biển miền Trung',scale:1.28,x:'0%',y:'-13%'},
    'Tây Nguyên':{slug:'tay-nguyen',label:'Tây Nguyên',description:'Cao nguyên, rừng thông và thác nước',scale:1.28,x:'0%',y:'-38%'},
    'Nam Bộ':{slug:'nam-bo',label:'Nam Bộ',description:'Miền sông nước và những đảo xanh',scale:1.28,x:'0%',y:'-65%'}
  };
  ...
  function renderRegionAreas(){
    if(state.regionView){regionAreas.innerHTML='';return;}
    ...
    const labels=Object.values(regionViews).map(r=>{const names=provinceGroups[r.label]||[];return `<div class="region-hover-labels" data-region="${r.slug}" aria-hidden="true">${names.map(name=>{const point=window.MapCatalog.anchors[name]||[50,50];return `<b style="left:${point[0]}%;top:${point[1]}%"><i aria-hidden="true"></i>${esc(name)}</b>`;}).join('')}</div>`;}).join('');
    regionAreas.innerHTML=hotspots+labels;
    regionAreas.querySelectorAll('.region-hotspot').forEach(button=>{const setHover=()=>{regionAreas.dataset.hoverRegion=button.classList[1]||'';};const clearHover=()=>{if(regionAreas.dataset.hoverRegion===button.classList[1])delete regionAreas.dataset.hoverRegion;};button.addEventListener('pointerenter',setHover);button.addEventListener('pointerleave',clearHover);button.addEventListener('focus',setHover);button.addEventListener('blur',clearHover);});
  ...
  function applyRegionView(){
    ...
    else if(hanoi){scene.style.removeProperty('--region-scale');scene.style.removeProperty('--region-x');scene.style.removeProperty('--region-y');toolbarTitle.textContent='Bản đồ Hà Nội';$('mapCrumb').textContent='Bản đồ quận, huyện · địa danh và khu vui chơi';$('backToCountry').textContent='← Bắc Bộ';$('backToCountry').hidden=false;provinceLayer.innerHTML='';hcmLayer.innerHTML='';daNangLayer.innerHTML='';renderHanoiMap();}
    else if(daNang){scene.style.removeProperty('--region-scale');scene.style.removeProperty('--region-x');scene.style.removeProperty('--region-y');toolbarTitle.textContent='Bản đồ Đà Nẵng';$('mapCrumb').textContent='Bản đồ khu vực · địa danh và khu vui chơi';$('backToCountry').textContent='← Trung Bộ';$('backToCountry').hidden=false;provinceLayer.innerHTML='';hcmLayer.innerHTML='';hanoiLayer.innerHTML='';renderDaNangMap();}
    ...
    else{scene.style.removeProperty('--region-scale');scene.style.removeProperty('--region-x');scene.style.removeProperty('--region-y');toolbarTitle.textContent='Việt Nam qua từng điểm đến';$('mapCrumb').textContent='Bản đồ toàn quốc · Chọn một khu vực trên ảnh';$('backToCountry').hidden=true;provinceLayer.innerHTML='';hcmLayer.innerHTML='';hanoiLayer.innerHTML='';daNangLayer.innerHTML='';}
    renderRegionAreas();
  }
  ...
  new ResizeObserver(()=>renderProvinceLabels()).observe(scene);
  let resizeTimer;
  addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{if(state.hcmView)renderHcmMap();else if(state.hanoiView)renderHanoiMap();else if(state.daNangView)renderDaNangMap();else if(state.provinceView)renderProvinceMap();},120);});
